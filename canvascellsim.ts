...
class CanvasCellSim<T>
{
    ...
    set(x: number, y:number, value:T): boolean
    {
        ...
        this.tryInit();
        if (Math.abs(2*x) > this.grid[0].length) this.updateGridWidth(Math.abs(2*x),  !was_init);
        if (Math.abs(2*y) > this.grid.length)    this.updateGridHeight(Math.abs(2*y), !was_init);

        let rx = x + this.offsetX;
        let ry = y + this.offsetY;

        if (rx < 0 || ry < 0 || rx >= this.grid[0].length || ry >= this.grid.length) return false;

        this.grid[ry][rx].value = value;
        return true;
    }

    ffwd(steps: number): void {
    ...
    updateGridWidth(calc: number, initial:boolean)
    {
        const curr = this.grid[0].length;
    ...
    updateGridHeight(calc: number, initial:boolean)
    {
        const curr = this.grid.length;
